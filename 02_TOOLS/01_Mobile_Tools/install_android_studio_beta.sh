...
#######################################################################
# File Name    : install_android_studio_beta.sh
# Description  : This script automates the installation and setup of
#                Android Studio AppImage. It downloads the latest
...
#                  chmod +x install_android_studio_beta.sh
#                • Run the script with:
#                  bash install_android_studio_beta.sh
...
download_appimage() {
    ...
    VERSION_URL=$(curl -sL "$APP_DOWNLOAD_URL" | grep -o '"browser_download_url": *"[^"]*"' | grep -o 'https://[^"]*android-studio-beta-.*.AppImage' | head -n 1)
    if [ -z "$VERSION_URL" ]; then
        echo "Error: Unable to find Android Studio AppImage URL."
        exit 1
    fi
...
# Run functions to download, extract, and set up Android Studio AppImage
download_appimage
extract_appimage
handle_extracted_files
...
HOME_DIR="$HOME_DIR"
version=\$(curl -sL "\$APP_DOWNLOAD_URL" | grep -o '"browser_download_url": *"[^"]*"' | grep -o 'https://[^"]*android-studio-beta-.*.AppImage' | head -n 1)
if [ -n "\$version" ]; then

...
########################################################################
# File Name    : install_apk_editor_studio.sh
# Description  : This script automates the installation and setup of
#                APK Editor Studio AppImage. It downloads the latest
#                version from GitHub, installs it, creates necessary
#                directories and symbolic links, provides updater and
#                remover scripts, and runs the AppImage in a bubblewrap
#                sandbox for security.
# Dependencies : curl, bubblewrap, rsvg-convert (optional for icon)
# Usage        : • Make the script executable with:
#                  chmod +x install_apk_editor_studio.sh
#                • Run the script with:
#                  bash install_apk_editor_studio.sh
#                • To update the AppImage, run:
#                  bash $HOME/.local/share/apk-editor-studio/update.sh
#                • To remove the AppImage, run:
#                  bash $HOME/.local/share/apk-editor-studio/remove.sh
########################################################################
...
APP_DOWNLOAD_URL="https://api.github.com/repos/kefir500/apk-editor-studio/releases"
...
download_appimage() {
    # Get the latest AppImage download link using GitHub API
    VERSION_URL=$(curl -sL "$APP_DOWNLOAD_URL" | grep -o '"browser_download_url": *"[^"]*"' | grep -o 'https://[^"]*apk-editor-studio_linux_.*.AppImage' | head -n 1)
    if [ -z "$VERSION_URL" ]; then
        echo "Error: Unable to find APK Editor Studio AppImage URL."
        exit 1
    fi
...
# Run functions to download, extract, and set up APK Editor Studio AppImage
download_appimage
extract_appimage
handle_extracted_files
...
HOME_DIR="$HOME_DIR"
version=\$(curl -sL "\$APP_DOWNLOAD_URL" | grep -o '"browser_download_url": *"[^"]*"' | grep -o 'https://[^"]*apk-editor-studio_linux_.*.AppImage' | head -n 1)
if [ -n "\$version" ]; then
